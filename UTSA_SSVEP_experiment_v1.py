...
window_handle = visual.Window([1920, 1080], fullscr=False, monitor='testMonitor', units='deg', screen=1)
...
square_left_number_frames = 3
square_right_number_frames = 5
square_up_number_frames = 1
square_down_number_frames = 2
square_face_number_frames = 3
...
run_program = True                 # The program will run for a whole trial using this flag
arrow_display = True               # Arrows start being displayed first, show fixation when value is False
...
# Manually put the first event which is command[0] = Up
...
for current_trial in range(number_of_trials):
    ...
    # --- Here is the program main loop: it will run until a trial has finished
    while run_program:
        ...
        while executed_command <= (len(command_sequence) - 1):
            ...
            if duration_elapsed:
                ...
                if time_flip_counter == 2:
                    ...
                    # Perform a event evaluation (every target duration instead of every screen flip)
                    #current_command = command_sequence[executed_command]
        ...
        # Command sequence is completed, stop the flickering
        run_program = False
    ...
    trial_text = 'Trial '+str(current_trial+1)+' has ended!. You will have '+str(after_trial_pause)+' seconds to rest'
    trial_indicator_text_stimulus.setAutoDraw(True)
    trial_indicator_text_stimulus.setText(trial_text)
...
time_reference_filename = "Time_0_"+str(reference_time[0])+"_"+str(reference_time[1])+"_"+str(reference_time[2])+".txt"
...
event_log_filename = "Event_log_"+str(reference_time[0])+"_"+str(reference_time[1])+"_"+str(reference_time[2])+".csv"
