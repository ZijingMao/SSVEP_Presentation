...
while countdown_time.getTime() > 0:
    recording_time = clock_object.getTime()

# Reset clock after the pause
clock_object.reset()
# ----------------------------------------------------------------------------------------------------------------------
...
window_handle = visual.Window(window_size, fullscr=window_full_screen, winType='pyglet', monitor='testMonitor', units='deg',
...
focus_fixation = visual.GratingStim(win=window_handle, mask='cross', name='bigFixation', size=1.6, pos=[0, 0], sf=0)
...
# Create the flickering and fixation clocks
flickering_clock_object = core.Clock()
fixation_clock_object = core.Clock()
time_tolerance = 0.01      # In seconds
# ----------------------------------------------------------------------------------------------------------------------
...
for current_trial in range(number_of_trials):
    ...
    # Reset the elapsed time for the current trial
    clock_object.reset()
    elapsed_time = 0
...
# ----------------------------------------------------------------------------------------------------------------------
        ''' Arrow OR Fixation has been displayed, check execution time and switch arrow/fixation '''

        # Get the elapsed time since the beginning of the trial, check for flickering and fixation duration
        elapsed_time = clock_object.getTime()
        flickering_duration_elapsed = (elapsed_time > flickering_display_time - time_tolerance) and \
                                      (elapsed_time < flickering_display_time + time_tolerance)

        fixation_duration_elapsed = (elapsed_time > fixation_display_time - time_tolerance) and \
                                    (elapsed_time < fixation_display_time + time_tolerance)

        # Check for the flickering time to complete (activates fixation and 'Cross' LSL)
        if arrow_display and flickering_duration_elapsed:
            ...
            # Send the UPCOMING event through LSL (next event is cross)
            LSL_send_command = 'Cross'

            # half of a command has now passed
            command_elapsed += 1

            # Send event through LSL
            outlet.push_sample([LSL_send_command])

        # Check if fixation time was completed (activates again arrow display based on the current command)
        elif not arrow_display and fixation_duration_elapsed:

            # Reset time counter
            clock_object.reset()
            elapsed_time = 0

            # Switch back to arrow display
            arrow_display = True

            # Send the UPCOMING event through LSL (depending on current command)
